...
export default function Comment({ postId, isScrollable }: { postId: number; isScrollable?: boolean }) {
   ...
   return (
      <Container className="comment">
         <CommentWrapper ref={commentListRef} selected={selectedComment !== undefined && selectedComment > 0} isScrollable={isScrollable}>
            {commentList?.map((item) => (
               <div key={item.id}>
                  <div className={`flex flex-col justify-center p-3 gap-1 ${selectedComment === item.id && 'selected'}`}>
         ...
         >
            {selectedComment !== undefined && selectedComment > 0 && <span className="w-fit text-sm text-zinc-500 bg-[#edf6ff] px-2 py-1 rounded-full">답글 작성 중 ...</span>}
...
const CommentWrapper = styled.div<{
   ...
   flex-direction: column;
   gap: 1rem;
   height: ${(props) => (props.isScrollable === false ? 'fit-content' : props.selected ? 'calc(95dvh - 11rem)' : 'calc(95dvh - 9.5rem)')};
   overflow: auto;
   overscroll-behavior: none;
`;
