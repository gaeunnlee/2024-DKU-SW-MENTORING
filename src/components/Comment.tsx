...
export default function Comment({ postId }: { postId: number }) {
   ...
   const [textareaRef, setTextareaRef] =
      useState<MutableRefObject<null | HTMLTextAreaElement>>();
   const lastReplyRef = useRef<HTMLDivElement>(null);
   const commentListRef = useRef<HTMLDivElement>(null);

   const fetchCommentList = () => {
      get({ api: `/post/mission-board/comments/${postId}`, auth: true }).then(
      ...
      <Container>
         ...
         >
            {selectedComment !== undefined && selectedComment > 0 && (
               <span className="w-fit text-sm text-zinc-500 bg-[#edf6ff] px-2 py-1 rounded-full">
                  답글 작성 중 ...
               </span>
            )}
...
const CommentWrapper = styled.div<{ selected: boolean }>`
   box-sizing: border-box;
   display: flex;
   flex-direction: column;
   gap: 1rem;
   height: ${(props) =>
      props.selected ? 'calc(95dvh - 11rem)' : 'calc(95dvh - 9.5rem)'};
   overflow: auto;
   overscroll-behavior: none;
`;
